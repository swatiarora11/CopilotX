...
import { ApiUser, ApiWorkOnTicketResponse, ErrorResult } from "../model/apiModel";
...
interface Response extends HttpResponseInit {
  status: number;
  jsonBody: {
    results: ApiUser[] | ApiWorkOnTicketResponse | ErrorResult;
  };
}
      ...
      case "POST": {
        ...
        switch (command) {
          case "workonticket": {  
            ...
            const result = await UserApiService.workOnTicket(ticketName, me.id, hours);
